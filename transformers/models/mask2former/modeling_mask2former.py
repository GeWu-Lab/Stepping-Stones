...
class Mask2FormerForUniversalSegmentationOutput(ModelOutput):
    ...
    loss_dict:Optional[List[Dict[str, torch.FloatTensor]]] = None
...
class Mask2FormerLoss(nn.Module):
    ...
    def avs_loss(self,
            masks_queries_logits: torch.Tensor,
            mask_labels: List[torch.Tensor],):
        pred_mask=nn.functional.interpolate(torch.mean(masks_queries_logits,dim=1).unsqueeze(1),(384,384)).squeeze(1)
        #target_masks, _ = self._pad_images_to_max_in_batch(mask_labels)
        target_masks=torch.stack([1-i[0] for i in mask_labels])#b*h*w
        bce=nn.BCEWithLogitsLoss(reduction="none")
        losses = {
            "loss_cross_entropy": 0,
            "loss_mask": bce(pred_mask.flatten(1),target_masks.flatten(1)).mean(),
            "loss_dice": dice_loss(pred_mask, target_masks.flatten(1), pred_mask.shape[0]),
        }
        return losses
    ...
    def forward(
        ...
        if all == 0:
            # mask = torch.tensor([True if i.shape[0] > 1 else False for i in class_labels])
            # mask_label = [i[1:, :, :] for i in mask_labels if i.shape[0] > 1]
            # class_label = [i[1:] for i in class_labels if i.shape[0] > 1]
            # masks_queries_logit = masks_queries_logits[mask]
            # class_queries_logit = class_queries_logits[mask]
            mask_label = mask_labels
            class_label = class_labels
            masks_queries_logit = masks_queries_logits
            class_queries_logit = class_queries_logits
            class_pred = torch.argmax(class_queries_logits, dim=-1)
            target_size = masks_queries_logit[0, 0].shape
            binary = torch.stack([torch.nn.functional.interpolate(
                mask_label[i][0].unsqueeze(dim=0).unsqueeze(dim=0), size=target_size, mode="nearest"
            )[0, 0] for i in range(len(mask_label))])
            binary = binary.unsqueeze(1).repeat(1, masks_queries_logit.shape[1], 1, 1)
            non_zero_positions = class_pred != 0
            masks_queries_logit[non_zero_positions] = masks_queries_logit[non_zero_positions] * binary[
                non_zero_positions]
            temp_mask = class_pred == 0
            masks_queries_logit[temp_mask] = masks_queries_logit[temp_mask] * binary[temp_mask]
        else:
            mask_label = mask_labels
            class_label = class_labels
            masks_queries_logit = masks_queries_logits
            class_queries_logit = class_queries_logits
        if is_avs:
            losses: Dict[str, Tensor] = self.avs_loss(masks_queries_logit, mask_label)
        else:
            indices = self.matcher(masks_queries_logit, class_queries_logit, mask_label, class_label)
            # compute the average number of target masks for normalization purposes
            num_masks = self.get_num_masks(class_label, device=class_label[0].device)
            # get all the losses
            losses: Dict[str, Tensor] = {
                **self.loss_masks(masks_queries_logit, mask_label, indices, num_masks, all,audio_boost_mask),
                **self.loss_labels(class_queries_logit, class_label, indices),
            }
        ...
        if auxiliary_predictions is not None:
            for idx, aux_outputs in enumerate(auxiliary_predictions):
                ...
                loss_dict = self.forward(masks_queries_logits, class_queries_logits, mask_labels, class_labels,is_avs=is_avs,all=all,audio_boost_mask=audio_boost_mask)
...
class Mask2FormerTransformerModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.num_feature_levels):
            ...
            if audio_boost_mask is not None:
                if torch.unique(audio_boost_mask).shape[0]<3:
                    low=0.4
                    high=0.6
                else:
                    low=0.1
        ...
        # print('query @ 2084-Line:', query_features.shape)  # [100, 1, 256]

        # prompt the query feature for segmentation
        if prompt_features_projected is not None:
            ...
            # print('query-pmp:', query_features.shape)  # [100, 1, 256]
...
class Mask2FormerModel(Mask2FormerPreTrainedModel):
    ...
    def forward(
            ...
            return_dict: Optional[bool] = None,
            audio_boost_mask: Optional[Tensor] = None,
            pixel_level_module_output= None,
            binary_gt: Optional[Tensor] = None
        ...
        if pixel_level_module_output is not None:
        ...
        # 有编码器的最后一层特征图，解码器的最后一层特征图（B*256*96*96），若output_hidden_states为True，还会返回编码器和解码器的多尺度特征图，前两项其实就是后两项的[-1]
